...
#if defined(NDEBUG)
# error "Euncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb6439b60ad089c9adeb4cd9950a9228535d938a9ddd3f8347f85241a3ad7e138");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Euncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Euncoin Signed Message:\n";
...
{
    // Euncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 400000000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 4000000); // Euncoin: 400M blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Euncoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Euncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Euncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Euncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xef04909f17fb2dcc122faa2bddf1b533a12bf0a4bd86e6c5338dbf7c2d488a99");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 05/Apr/2018 Facebook scandal 'hit 87 million users'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eddca0f10b566489243a7875c3b33da4f63fd80ac61a8cabff6c19851ab75ca6fe3f6b1494d244f02fb8151ddb4d3946de687a4e60fb92dad25fb8e4e90308b6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522926890;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084912614;

        if (fTestNet)
        {
            block.nTime    = 1522926850;
            block.nNonce   = 386721666;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9a1e3386b4ee82245608e8a210d088f65f565360c2af13cd79a085d4b17e526c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc9, 0xb7, 0xd6 }; // Euncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EuncoinMiner(CWallet *pwallet)
{
    printf("EuncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("euncoin-miner");
    ...
    try { loop {
        ...
        printf("Running EuncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EuncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EuncoinMiner, pwallet));
}
